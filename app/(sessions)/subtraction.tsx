...
import { SubtractionExercise } from '../../src/components/exercises/SubtractionExercise';
...
export default function SubtractionSession() {
  ...
  const [mascotMsg, setMascotMsg] = useState('What is left?');
  ...
  useEffect(() => {
    if (!question) return;
    const msg = `What is ${question.a} minus ${question.b}?`;
    setMascotMsg(msg);
    setMascotMood('happy');
    setHintUsed(false);
    setDisabled(false);
  ...
  const handleAnswer = useCallback((value: number, usedVoice: boolean) => {
    ...
    progress.recordAnswer('subtraction', correct);
