...
import { CountingExercise } from '../../src/components/exercises/CountingExercise';
...
import type { CountingQuestion } from '../../src/types/exercises';

export default function CountingSession() {
  ...
  const [mascotMsg, setMascotMsg] = useState('How many do you see?');
  ...
  const question = session.questions[session.currentIndex] as CountingQuestion | undefined;

  useEffect(() => {
    if (!question) return;
    const msg = `How many ${question.emoji} do you see?`;
    setMascotMsg(msg);
    setMascotMood('happy');
    setHintUsed(false);
    setDisabled(false);
    speak(msg, settings.voiceSpeed);
  }, [session.currentIndex, question?.count]);
  ...
  const handleAnswer = useCallback((value: number, usedVoice: boolean) => {
    ...
    if (correct) {
      ...
      const newBadges = checkAndAwardBadges('counting', session.difficulty);
      ...
      setTimeout(() => {
        ...
        if (session.currentIndex + 1 >= session.questions.length) {
          progress.completeSession('counting');
      ...
      if (session.wrongAttempts >= 1) {
        setHintUsed(true);
        const hintMsg = pickRandom(HINT_OFFER);
        setTimeout(() => { setMascotMsg(hintMsg); speak(hintMsg, settings.voiceSpeed); }, 1200);
      }
