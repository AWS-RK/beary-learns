...
export function SubtractionExercise({
  ...
  const [crossedOut, setCrossedOut] = React.useState(false);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.questionCard}>
        ...
        {config.showVisualAid && (
          <View style={styles.dotsRow}>
            {Array.from({ length: question.a }).map((_, i) => (
              <Text
                key={i}
                style={[styles.dot, (crossedOut && i >= question.a - question.b) && styles.crossedDot]}
              >
                {crossedOut && i >= question.a - question.b ? '❌' : '🟣'}
              </Text>
            ))}
          </View>
        )}

        {config.showDotsAlways && !crossedOut && (
          <BigButton
            label="Cross them out! ❌"
            onPress={() => setCrossedOut(true)}
            color={colors.accentPurple + '33'}
...
const styles = StyleSheet.create({
  ...
  operator: { fontFamily: fonts.extraBold, fontSize: fontSize.xxl, color: colors.accentPurple },
  unknown: { fontFamily: fonts.extraBold, fontSize: fontSize.display, color: colors.primary },
  dotsRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, justifyContent: 'center' },
  dot: { fontSize: 24 },
  crossedDot: { opacity: 0.4 },
